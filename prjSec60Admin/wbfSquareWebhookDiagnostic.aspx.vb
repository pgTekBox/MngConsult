...
''' <summary>
''' Page de diagnostic des webhooks Square (console d'administration).
'''
''' URL : wbfSquareWebhookDiagnostic.aspx
'''
''' Affiche :
'''   - Stats agregees (total, processed, failed, etc.)
'''   - Liste des events recents avec filtres (status, type, periode)
'''   - Details deroulants par event (payload JSON, ErrorMessage, merchant)
'''
''' Source : table dbo.tSquareEvenement (journal des webhooks Square).
''' Procedures : s0700GetSquareEvents / s0701GetSquareEventStats (DB partagee).
''' Acces : reserve aux administrateurs (garde globale de clsData.OnLoad).
''' </summary>
Public Class wbfSquareWebhookDiagnostic
    Inherits clsData
    ...
    Private Sub LoadStats(sinceHours As Integer)
        Try
            ...
            Dim ds As DataSet = ExecuteSQLds("s0701GetSquareEventStats", p)
    ...
    Private Sub LoadEvents(status As String, eventType As String, sinceHours As Integer)
        Try
            ...
            Dim ds As DataSet = ExecuteSQLds("s0700GetSquareEvents", p)
            ...
            For Each row As DataRow In ds.Tables(0).Rows
                ...
                Dim eventId As String = If(row("SquareEventId") Is DBNull.Value, "", row("SquareEventId").ToString())
                ...
                Dim merchantId As String = If(row("MerchantId") Is DBNull.Value, "", row("MerchantId").ToString())
                Dim squareCreated As Object = row("SquareCreatedAt")
                ...
                If Not String.IsNullOrEmpty(merchantId) Then
                    sb.Append("<div class=""detail-row""><span class=""lbl"">Merchant ID</span><span class=""val"">" & Server.HtmlEncode(merchantId) & "</span></div>")
                End If
                If Not IsDBNull(squareCreated) AndAlso squareCreated IsNot Nothing Then
                    sb.Append("<div class=""detail-row""><span class=""lbl"">Créé chez Square</span><span class=""val"">" & CDate(squareCreated).ToString("dd MMMM yyyy HH:mm:ss", culture) & "</span></div>")
                End If
                If receivedOn <> Date.MinValue Then
