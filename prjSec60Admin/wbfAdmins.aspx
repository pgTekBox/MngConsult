<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/Site.Master"
    MaintainScrollPositionOnPostback="true" CodeBehind="wbfAdmins.aspx.vb"
    Inherits="prjSec60Admin.wbfAdmins" %>

<asp:Content ID="cTitle" ContentPlaceHolderID="TitleContent" runat="server">
    Administrateurs — Sec60Admin
</asp:Content>
...
<asp:Content ID="cHead" ContentPlaceHolderID="HeadContent" runat="server">
    ...
    <style>
        .listview-list-head {
            display: grid;
            grid-template-columns: 50px minmax(220px, 1fr) minmax(180px, 1fr) 90px 150px 100px;
...
<asp:Content ID="cMain" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <telerik:RadWindowManager ID="rwmAdmins" runat="server" EnableShadow="true">
        <Windows>
            <telerik:RadWindow ID="rwAdmin" runat="server"
                Modal="true" VisibleOnPageLoad="false"
                Behaviors="Close,Move,Resize" DestroyOnClose="true"
                Width="640px" Height="600px" Title="Administrateur"
                OnClientClose="rwAdmin_OnClientClose" ClientIDMode="Static">
            </telerik:RadWindow>
        </Windows>
    ...
    <telerik:RadAjaxManager ID="Ram1" runat="server" ClientIDMode="Static">
        <AjaxSettings>
            <telerik:AjaxSetting AjaxControlID="btnSearch">
                <UpdatedControls><telerik:AjaxUpdatedControl ControlID="rlvAdmins" /></UpdatedControls>
            </telerik:AjaxSetting>
    ...
    <div class="page-head">
        ...
        <div class="searchbox">
            <asp:Button ID="btnAddAdmin" runat="server"
                CssClass="btn btnAddRow" Text="+ Ajouter un administrateur"
                CausesValidation="false"
                OnClientClick="openAdminWindow(0); return false;" />

            <div class="search-group">
                <asp:TextBox ID="tbSearch" runat="server"
                    CssClass="input txttbsearch" placeholder="Rechercher (nom, courriel)…" />
                <asp:Button ID="btnSearch" runat="server" CssClass="btn btn-icon btn-icon-search" Text="" />
                <asp:Button ID="btnClear" runat="server" CssClass="btn btn-icon btn-icon-clear"
                    Text="" ToolTip="Effacer" CausesValidation="false" />
            </div>
        </div>
    </div>
    ...
    <div class="full-grid">
        <div class="list-shell">
            ...
            <telerik:RadListView ID="rlvAdmins" runat="server"
                Skin="Metro" DataKeyNames="Id" AllowPaging="false"
                    ...
                    <div class="listview-row">
                        ...
                        <div class="listview-actions">
                            <asp:Button ID="btnEdit" runat="server"
                                CssClass="btn btn-icon btn-icon-edit" Text="" ToolTip="Modifier"
                                CausesValidation="false"
                                OnClientClick='<%# "openAdminWindow(" & Eval("Id") & "); return false;" %>' />
                            <asp:Button ID="btnDelete" runat="server"
                                CssClass="btn btn-icon btn-icon-delete" Text="" ToolTip="Supprimer"
                                CommandName="DeleteAdmin" CommandArgument='<%# Eval("Id") %>'
                ...
                <EmptyDataTemplate>
                    <div class="listview-empty">Aucun administrateur trouvé.</div>
                </EmptyDataTemplate>
    ...
    <script type="text/javascript">
        function openAdminWindow(id) {
            var win = $find('rwAdmin');
            if (!win) return;
            win.setUrl('wbfAdminEdit.aspx?id=' + id);
            win.show();
            win.center();
            win.setActive(true);
        }
        function rwAdmin_OnClientClose(sender, args) {
            var ajaxManager = $find('Ram1');
            if (ajaxManager) { ajaxManager.ajaxRequest('refreshgrid'); }
        }
    </script>
</asp:Content>
