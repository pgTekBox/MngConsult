...
''' <summary>
''' Connecteur KYB (Know Your Business). Interface abstraite IKybProvider +
''' fournisseur SANDBOX simulé (règles déterministes). Le vrai fournisseur
''' (Trulioo/Onfido) implémenterait la même interface, gaté par le contrat.
''' clsKyb.RunCheck orchestre : rassemble les données de l'abonné, appelle le
''' fournisseur, enregistre la vérification (T057), pilote le StatutKYB et
''' journalise l'action (audit).
''' </summary>
...
''' <summary>Fournisseur SANDBOX : résultat déterministe selon les données.
''' - watchlist NON claire si la raison sociale contient « REJECT » ou si le
'''   numéro d'entreprise se termine par 0000 (déclencheurs de test) ;
''' - registre trouvé si numéro d'entreprise renseigné ;
''' - adresse valide si ville + code postal renseignés.</summary>
...
Public Class clsKyb
    ...
    Public Shared Function GetProvider() As IKybProvider
        ...
        Select Case name
            ' Case "trulioo" : Return New TruliooKybProvider()   ' gaté fournisseur
    ...
    Private Shared Function MapStatut(status As String) As String
        Select Case status
            ...
            Case Else : Return "EnCours"   ' Review / autres
        End Select
    End Function
