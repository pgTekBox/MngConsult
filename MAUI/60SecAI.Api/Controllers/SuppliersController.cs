...
/// <summary>
/// Factures fournisseur. Même flux que les factures client (SalesController) :
/// liste (s0023GetSuppliersInvoices), détail (s0038/s0039) et « Payé »
/// (s0714GetSupplierInvoicePaid, sens DECAISSEMENT).
/// </summary>
[ApiController]
...
public class SuppliersController : ControllerBase
{
	...
	{
		"PAYEE" => "Collected",   // payée
		"EN_RETARD" => "Overdue", // en retard
		_ => "Receivable",        // OUVERTE, PARTIELLE… = à payer
	};

	/// <summary>
	/// Liste des factures fournisseur (s0023GetSuppliersInvoices), optionnellement filtrées par status
	/// (Collected | Overdue | Receivable). Sans filtre = toutes.
	/// </summary>
	[HttpGet("invoices")]
	...
	/// <summary>Résumé fournisseur : Payé / En retard / À payer (mêmes buckets que le client).</summary>
	[HttpGet("summary")]
	...
	/// <summary>Liste des fournisseurs pour le sélecteur (s0011Getsuppliers).</summary>
	[HttpGet("lookup")]
	public async Task<ActionResult<IEnumerable<ClientLookupDto>>> GetSuppliers()
	{
		var list = new List<ClientLookupDto>();

		await using var conn = new SqlConnection(_connectionString);
		await conn.OpenAsync();
		await using var cmd = new SqlCommand("s0011Getsuppliers", conn) { CommandType = CommandType.StoredProcedure };
	...
	/// <summary>
	/// Crée une facture fournisseur (brouillon) via s0040SaveInvoiceItems (@DocumentType='FOURNISSEUR',
	/// DocumentTypeId=2). Même TVP et même logique que la facture client.
	/// </summary>
	[HttpPost("invoices")]
	public async Task<ActionResult<CreateInvoiceResult>> CreateInvoice([FromBody] CreateInvoiceRequest request)
	{
		...
		cmd.Parameters.AddWithValue("@DocumentType", "FOURNISSEUR");
		cmd.Parameters.AddWithValue("@CompanyGUID", CompanyGuid);
	...
	/// <summary>Détail d'une facture fournisseur : en-tête (s0038) + lignes (s0039) + payé (s0714).</summary>
	...
	public async Task<ActionResult<InvoiceDetailDto>> GetInvoice(int id)
	{
		...
		// ----- En-tête -----
		string number = string.Empty, name = "Fournisseur", address = string.Empty, note = string.Empty, po = string.Empty;
		...
		await using (var cmd = new SqlCommand("s0038GetInvoiceById", conn) { CommandType = CommandType.StoredProcedure })
		{
			...
			if (await reader.ReadAsync())
			{
				...
				name = StripHtml(ReadString(reader, Ordinal(reader, cols, "Name", "DisplayName"), "Fournisseur"));
		...
		// ----- Payé / Reste à payer (DECAISSEMENT) -----
		decimal paid = 0;
		await using (var cmd = new SqlCommand("s0714GetSupplierInvoicePaid", conn) { CommandType = CommandType.StoredProcedure })
		{
			cmd.Parameters.AddWithValue("@InvoiceId", id);
			var result = await cmd.ExecuteScalarAsync();
