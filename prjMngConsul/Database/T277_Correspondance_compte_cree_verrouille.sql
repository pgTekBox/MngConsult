-- =============================================================================
-- T277 — Un compte déjà créé au plan ne se rediscute pas à l'étape 2
--
-- Après l'étape 3, la ligne « Créer » porte le compte qu'elle a fait naître
-- (PlanComptableId). L'étape 2 la réaffichait pourtant comme une décision
-- ouverte — « Créer », avec 2080 dans la liste — et un simple « Enregistrer »
-- la renvoyait : s0757 (T276) y voyait alors un « Créer » sur un numéro qui
-- existe déjà, et refusait toute la page.
--
-- Désormais une ligne créée est hors de portée de s0757 : ce que la page
-- renvoie pour elle est ignoré, la décision reste ce qu'elle est. La page,
-- de son côté, la montre « créée » et verrouille ses champs.
-- Le reste de s0757 est celui de T276.
-- =============================================================================
...
BEGIN
    ...
    SELECT TOP 1 @faute = N'Le compte ' + d.CompteCible + N' est déjà '
...
PRINT 'T277_Correspondance_compte_cree_verrouille.sql : terminé.';
